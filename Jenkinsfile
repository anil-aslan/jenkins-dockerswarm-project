    ...
    stages {
        ...
        stage('Deploy App on Docker Swarm'){
            ...
            steps {
                ...
                sleep(10)
                sh 'mssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no --region ${AWS_REGION} ${MASTER_INSTANCE_ID} docker stack deploy --with-registry-auth -c ${HOME_FOLDER}/
jenkins-dockerswarm-project/docker-compose.yml ${APP_NAME}'
            }
        }
    }
    post {
